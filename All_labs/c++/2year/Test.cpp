...
int main() {
    Node *head = new Node();
    Node *list = new Node();  // Инициализация списка "list"
    list->Next = NULL;  // Убедимся, что Next указывает на NULL для конца списка
    head->Next = NULL;  // Тоже самое для head
    string s, s1;
    
    while (true) {
        cin >> s;
        if (s == "exit") {
            Node* end = list->Next;  // Переходим ко второму элементу списка "list"
            while (end != NULL) {  // Выводим значения до конца списка
                cout << end->value << endl;
                end = end->Next;
            }
            cout << "goodbye";
